...
export const fetchByResourceClass = async (id: number) => {
    const response = await fetch(apiUrl + 'items?resource_class_id=' + id + '&per_page=500', header
...
export const fetchByResourceTemplate = async (id: number) => {
    const response = await fetch(apiUrl + 'items?resource_template_id=' + id + '&per_page=500', header)
    return  await response.json()
}
